...
class Kalpcontract {
    ...
    */
    ...
    async afterTransaction(ctx) {
        // default implementation is do nothing
        logger.info("After Transaction:", ctx.stub.getTxID(), "IsPayable is:", this.IsPayableContract);
        const {fcn, params} = ctx.stub.getFunctionAndParameters()
        
        if  (this.IsPayableContract) {
            logger.debug("IsPayableContract is true")
            let paymentTracker = JSON.parse(params) 
            logger.debug("parse payment info complete", paymentTracker)
